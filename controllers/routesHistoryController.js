...
// Función para obtener todo el historial de rutas
const getRoutesHistory = async (req, res) => {
  try {
    ...
    res.json(routesHistory[0]);  // Devuelve el historial de rutas en formato JSON
  } catch (error) {
...
// Función para crear una nueva entrada en el historial de rutas
const createRouteHistory = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json(newRouteHistory[0]);  // Devuelve la nueva entrada de historial creada
  } catch (error) {
...
// Función para actualizar una entrada del historial de rutas
const updateRouteHistory = async (req, res) => {
  const { History_ID } = req.params;
  const { User_ID, Route_ID } = req.body;

  try {
    // Buscar la entrada por History_ID
    ...
    res.json({ message: 'Historial de ruta actualizado correctamente' });  // Confirmación de actualización
  } catch (error) {
...
// Función para eliminar una entrada del historial de rutas
const deleteRouteHistory = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: 'Historial de ruta eliminado con éxito' });  // Confirmación de eliminación
  } catch (error) {
